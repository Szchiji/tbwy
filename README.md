...
- `/admin` - 获取最新10条帖子的管理员链接
- `/admin <post_id>` - 获取指定帖子的管理员链接
- `/notice <内容>` - 更新网站公告
...
13. **性能优化**:
    - 数据库索引优化
    - 分页查询提升加载速度
14. **管理员链接命令** (2026-01-16 新增):
    - `/admin` 命令获取最新10条帖子的管理员链接
    - `/admin <post_id>` 命令获取指定帖子的管理员链接
    - 新帖子发布到频道时自动向管理员发送管理员链接
    - 管理员链接包含 `admin_key` 参数，便于快速进入管理模式
